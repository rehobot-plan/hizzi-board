...
export default function TodoItem({ post, canEdit }: TodoItemProps) {
  ...
  const taskBtnStyle = (t: 'work' | 'personal'): React.CSSProperties => {
    ...
    if (t === 'work') return { fontSize: 11, padding: '5px 13px', borderRadius: 3, cursor: 'pointer', border: `1px solid ${isOn ? '#C17B6B' : 'rgba(193,123,107,0.35)'}`, color: isOn ? '#C17B6B' : 'rgba(193,123,107,0.45)', background: isOn ? '#FFF5F2' : 'transparent', transition: 'all 0.15s ease' };
    return { fontSize: 11, padding: '5px 13px', borderRadius: 3, cursor: 'pointer', border: `1px solid ${isOn ? '#7B5EA7' : 'rgba(123,94,167,0.35)'}`, color: isOn ? '#7B5EA7' : 'rgba(123,94,167,0.45)', background: isOn ? '#F0ECF5' : 'transparent', transition: 'all 0.15s ease' };
  };
  ...
  const renderDetailAttachment = () => {
    if (detailRemoveAttachment) return (
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <span onClick={() => setDetailRemoveAttachment(false)} style={{ fontSize: 10, color: '#C17B6B', cursor: 'pointer', border: '1px solid #C17B6B', padding: '1px 7px', borderRadius: 2 }}>취소</span>
      </div>
    );
    ...
    if (detailNewFile) return (
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <span style={{ flex: 1, fontSize: 12, color: '#C17B6B' }}>{detailNewFile.name}</span>
        <span onClick={() => setDetailNewFile(null)} style={{ fontSize: 10, color: '#9E8880', cursor: 'pointer' }}>취소</span>
      </div>
    );
    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <span style={{ fontSize: 12, color: '#C4B8B0', flex: 1 }}>없음</span>
        <span onClick={() => detailFileInputRef.current?.click()} style={{ fontSize: 10, color: '#C17B6B', border: '1px solid #C17B6B', padding: '1px 7px', borderRadius: 2, cursor: 'pointer' }}>+ 추가</span>
      </div>
    );
  };
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 4 }}>
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              {post.requestFrom && post.requestContent && (
                <div style={{ fontSize: 11, color: '#C4B8B0', marginTop: 2, lineHeight: 1.4, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{post.requestContent}</div>
              )}
            </div>
          ...
          <div style={{ display: 'flex', gap: 4, marginTop: 4, alignItems: 'center', flexWrap: 'wrap' }}>
              ...
              return (
                <span style={{ position: 'relative', display: 'inline-block' }}>
                  ...
                  {label === '특정' && showSpecificTooltip && (
                    <div style={{ position: 'absolute', top: 'calc(100% + 4px)', left: 0, background: '#fff', border: '0.5px solid #EDE5DC', padding: '6px 8px', zIndex: 50, whiteSpace: 'nowrap' }}>
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, maxWidth: 240 }}>
                        {post.visibleTo.map(email => {
                          const u = users.find(u => u.email === email);
                          return <span key={email} style={{ fontSize: 9, padding: '2px 7px', background: '#F5F0EE', color: '#9E8880', letterSpacing: '0.04em' }}>{u?.name || email.split('@')[0]}</span>;
                        })}
                      </div>
                    </div>
                  )}
            ...
            {post.requestFrom && (
              <>
                <span style={{ fontSize: 9, padding: '2px 7px', background: '#FCEEE9', color: '#A0503A', letterSpacing: '0.04em' }}>
                ...
                {post.visibleTo && post.visibleTo.length > 1 && (
                  <div style={{ position: 'relative', display: 'inline-flex', alignItems: 'center' }}
                    ...
                    <span style={{ fontSize: 9, padding: '2px 7px', background: '#F5F0EE', color: '#9E8880', letterSpacing: '0.04em', cursor: 'default' }}>TEAM</span>
                    {showTeamTooltip && typeof window !== 'undefined' && createPortal(
                      <div style={{ position: 'fixed', top: tooltipPos.top, left: tooltipPos.left, background: '#fff', border: '0.5px solid #EDE5DC', padding: '8px', zIndex: 9999, boxShadow: '0 4px 12px rgba(44,20,16,0.08)' }}>
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, auto)', gap: 4 }}>
                          {post.visibleTo.map(email => {
                            const u = users.find(u => u.email === email);
                            return <span key={email} style={{ fontSize: 9, padding: '3px 8px', background: '#F5F0EE', color: '#9E8880', letterSpacing: '0.04em', whiteSpace: 'nowrap' }}>{u?.name || email.split('@')[0]}</span>;
                          })}
                        </div>
                      </div>,
                      document.body
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(44,20,16,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}
          ...
          <div style={{ background: '#fff', border: '1px solid #EDE5DC', borderRadius: 6, width: '100%', maxWidth: 480, maxHeight: '90vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}
            ...
            <div style={{ background: '#5C1F1F', padding: '14px 20px 12px', flexShrink: 0 }}>
              ...
              {isEditingTitle ? (
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <span style={{ fontSize: 15, fontWeight: 700, color: '#FDF8F4', lineHeight: 1.4 }}>{detailTitle}</span>
                  {canEdit && (
            ...
            <div style={{ background: '#FDF8F4', borderBottom: '1px solid #EDE5DC', padding: '7px 20px', display: 'flex', alignItems: 'center', flexShrink: 0 }}>
              ...
              <span style={{ fontSize: 10, padding: '2px 7px', borderRadius: 3, background: 'none', color: detailVisibility === 'all' ? '#3B6D11' : detailVisibility === 'me' ? '#185FA5' : '#854F0B', border: `1px solid ${detailVisibility === 'all' ? '#639922' : detailVisibility === 'me' ? '#378ADD' : '#BA7517'}` }}>
            ...
            <div style={{ background: '#FDF8F4', borderTop: '1px solid #EDE5DC', padding: '11px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
              <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                ...
                <span onClick={async () => { await handleDelete(); setShowDetailModal(false); }}
                  style={{ fontSize: 12, color: '#C17B6B', border: '1px solid #C17B6B', background: 'none', padding: '5px 12px', borderRadius: 3, cursor: 'pointer' }}>삭제</span>
              </div>
              <span onClick={!isSaving ? handleDetailSave : undefined}
                style={{ fontSize: 12, fontWeight: 700, padding: '6px 18px', borderRadius: 3, background: isSaving ? '#9E8880' : '#2C1810', color: '#FDF8F4', cursor: isSaving ? 'not-allowed' : 'pointer', transition: 'background 0.15s ease' }}>
                {isSaving ? '저장 중...' : '저장'}
              </span>
            </div>
          </div>
              ...
              <div style={{ width: 260, flexShrink: 0, borderRight: '1px solid #EDE5DC', padding: 20, display: 'flex', flexDirection: 'column', gap: 16, overflowY: 'auto' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    <span style={{ fontSize: 9, padding: '3px 8px', color: !post.visibleTo || post.visibleTo.length === 0 ? '#3B6D11' : post.visibleTo.length > 1 ? '#854F0B' : '#185FA5', border: !post.visibleTo || post.visibleTo.length === 0 ? '1px solid #639922' : post.visibleTo.length > 1 ? '1px solid #BA7517' : '1px solid #378ADD' }}>
                ...
                <div>
                  ...
                  {post.attachment ? (
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 12px', border: '0.5px solid #EDE5DC', background: '#fff' }}>
                      ...
                      <a href={post.attachment.url} target="_blank" rel="noopener noreferrer" style={{ fontSize: 10, color: '#C17B6B', textDecoration: 'none' }}>열기</a>
                    </div>
                  ) : (
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(44,20,16,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: '#fff', border: '1px solid #EDE5DC', width: '100%', maxWidth: 480, zIndex: 1001 }}>
            ...
            <div style={{ padding: '12px 20px', borderTop: '1px solid #EDE5DC', background: '#FDF8F4', display: 'flex', justifyContent: 'space-between' }}>
              <button onClick={() => setIsEditOpen(false)} style={{ fontSize: 10, color: '#9E8880', background: 'none', border: 'none', cursor: 'pointer' }}>취소</button>
              <button onClick={handleEditSave} disabled={isUpdating || uploading}
                style={{ fontSize: 10, padding: '8px 20px', background: '#2C1810', color: '#FDF8F4', border: 'none', cursor: isUpdating ? 'not-allowed' : 'pointer' }}>
