...
export default function LeavePage() {
  ...
  const leaveTableData = useMemo(() => {
    ...
    return employees.map((emp) => {
      ...
      const remaining = (isNaN(total) ? 0 : total) - totalUsed;
      return {
        id: emp.id,
        name: emp.name,
  ...
  if (leaveViewPermission === 'none') {
    return (
      <div className="min-h-screen bg-[#FDF8F4] p-8">
        <button onClick={() => router.push('/')} className="mb-8 text-sm text-[#9E8880] hover:text-[#2C1810]">← 돌아가기</button>
            ...
            <>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                ...
                <div className="border border-[#EDE5DC] p-4 rounded bg-white">
                  ...
                  <div className="text-3xl font-bold" style={{ color: currentUserData.remain < 0 ? '#C17B6B' : '#2C1810' }}>
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr style={{ borderBottom: '1px solid #EDE5DC', background: '#FDF8F4' }}>
                  <th className="px-3 py-2 text-left font-semibold text-[#2C1810]" style={{ fontSize: '11px' }}>이름</th>
                  <th className="px-3 py-2 text-center font-semibold text-[#2C1810]" style={{ fontSize: '11px' }}>입사일</th>
                  <th className="px-3 py-2 text-center font-semibold text-[#2C1810]" style={{ fontSize: '11px' }}>발생</th>
                  <th className="px-3 py-2 text-center font-semibold text-[#2C1810]" style={{ fontSize: '11px' }}>수동입력</th>
                  <th className="px-3 py-2 text-center font-semibold text-[#2C1810]" style={{ fontSize: '11px' }}>예정</th>
                  <th className="px-3 py-2 text-center font-semibold text-[#2C1810]" style={{ fontSize: '11px' }}>확정사용</th>
                  <th className="px-3 py-2 text-center font-semibold text-[#2C1810]" style={{ fontSize: '11px' }}>총사용</th>
                  <th className="px-3 py-2 text-center font-semibold text-[#2C1810]" style={{ fontSize: '11px' }}>잔여</th>
                </tr>
              </thead>
              <tbody>
                {leaveTableData.map((row) => (
                  <tr key={row.id} style={{ borderBottom: '1px solid #EDE5DC' }}>
                    <td className="px-3 py-2 text-[#2C1810]" style={{ fontSize: '11px' }}>{row.name}</td>
                    <td className="px-3 py-2 text-center text-[#2C1810]" style={{ fontSize: '11px' }}>{row.joinDate}</td>
                    <td className="px-3 py-2 text-center text-[#2C1810]" style={{ fontSize: '11px' }}>{row.total}</td>
                    <td className="px-3 py-2 text-center text-[#2C1810]" style={{ fontSize: '11px' }}>{row.manualUsed}</td>
                    <td className="px-3 py-2 text-center text-[#2C1810]" style={{ fontSize: '11px' }}>{row.plannedUsed}</td>
                    <td className="px-3 py-2 text-center text-[#2C1810]" style={{ fontSize: '11px' }}>{row.confirmedUsed}</td>
                    <td className="px-3 py-2 text-center text-[#2C1810]" style={{ fontSize: '11px' }}>{row.totalUsed}</td>
                    <td className="px-3 py-2 text-center" style={{ fontSize: '11px', color: row.remaining < 0 ? '#C17B6B' : '#2C1810', fontWeight: row.remaining < 0 ? 'bold' : 'normal' }}>
                      {row.remaining}
                    </td>
                  </tr>
                ))}
