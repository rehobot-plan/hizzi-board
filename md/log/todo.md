...
- **`NEXT_PUBLIC_FIREBASE_API_KEY` 의 `Development` 행이 없다** (2026-08-19 실측 · 나머지 다섯은 있다). Vercel `Settings → Environment Variables` 자리이고 저장소에서 못 닫는다. **미리보기·프로덕션과는 무관하다** — 그 둘은 그 값을 갖고 있다.
