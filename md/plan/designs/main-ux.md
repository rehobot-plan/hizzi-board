...
- **5 달력 피어 탭** — **⑤-1 완료(세션 #70, 2분할 재시도 후)**. 본인 패널 월 그리드 + scope 이진 토글 + 수평 스와이프 + FAB 분기 + /mydesk/calendar legacy redirect. 타인 패널은 "이 패널의 달력은 준비 중입니다" placeholder. **⑤-3 후속**: 타인 패널 scope/privacy 정제 (visibleTo 기반 접근 제어 모델 재설계, viewer/owner/admin 권한 매트릭스) — 기획 대화 선행 후 독립 세션 착수.
