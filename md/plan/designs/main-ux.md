...
### 2.5. 활성 + 최근 완료 회색 영역 (self-overrule · 세션 #67~)

활성 + 최근 완료 회색 영역 (collapsible 기본 접힘, 최근 5개 한도). 체크 시 즉시 사라지지 않고 회색 영역에 잔류(line-through·회색 글씨), 24시간 경과 시 RecordModal 보관 이관. 메모는 completed 없으므로 미적용. 삭제는 단호한 의도라 즉시 제거 유지(스와이프 + 5초 토스트 → 휴지통). chillkim 님 self-overrule (세션 #67~).
...
- [ ] 회색 영역 펼침 시 개별 복원·영구 완료 처리 액션 노출
...
FAB 빠른 추가 모달은 *요청 옵션 부재*. 요청은 채팅 시나리오 3·4 주류 입구로 일원화 — 본인 일정·할일·메모는 FAB·채팅 양 입구 수용, 요청은 채팅 단일 입구.
...
요청 추가는 채팅이 *주류*. FAB는 본인 일정·할일·메모 한정 — 두 입구의 의도된 분기.
...
- 신규: `src/store/chatInputStore.ts` — 대화 상태 관리 (zustand) · ChatMessage Firestore I/O · derivedIds cascade
- 신규: `src/lib/parseIntent.ts` — ai-capture-hb.md 7.1 계약 + LLM 2단 stub
- 신규: `src/lib/parseLocal.ts` — 1단 규칙 파서 (4축 + 6인 매칭 표 + confidence 5단계 + 복수 항목 감지)
- 수정: `src/app/(main)/page.tsx` — 홈 상단 배치 (기존 CreatePost 상단 영역 제거는 4.2 범위)
- 수정: `firestore.rules` — chatMessages 블록 추가 (hard delete 차단)
...
- **6 홈 채팅 기반 입력** — **첫 구현 완료(세션 #64)**. 시나리오 1~3 완전 구현 + 시나리오 4 항목 카드 UI + 승격 placeholder. ai-capture-hb.md 단일 출처. LLM 2단 stub · 수신자·기한·타입 unset 질의는 확장 포인트로만 유지.
- **2 완료·삭제 대칭** — **완료**. 2층까지(세션 #60, RecordModal·회수 링크) + **3층 완결(세션 #70)**: 탭 행 우상단 ··· 드롭다운 → "기록" 진입점, RecordModal windowFilter='all'. visibleTo 공유 필터(postSelectors.canViewPost)로 1·2층 기존 권한 누락까지 동시 해소. FLOW 1 요청 cascade 복구는 postStore restorePost + todoRequestStore.reactivateRequest 일반화 경로로 3층 진입도 자동 커버.
...
- **5 달력 피어 탭** — **완결**. ⑤-1(세션 #70): 본인 패널 월 그리드 + scope 이진 토글 + 수평 스와이프 + FAB 분기 + /mydesk/calendar legacy redirect + 타인 패널 placeholder. **⑤-3(2026-04-25, 6ca2454)**: 타인 패널 placeholder 제거 + 월 그리드 read-only · members=[panelOwnerEmail] 강제 · panelVisitingViewer 컨텍스트 도입 · visibility='me' 전면 차단(admin 포함) · specific은 visibleTo 명시 시 viewer 체크 · requestId(todoRequest cascade) 양당사자/visibleTo strict · leaveEvents는 panelOwner 매칭만 · 쓰기 진입점(FAB·onDateClick·onDateRangeSelect·canEdit) 전면 차단.
