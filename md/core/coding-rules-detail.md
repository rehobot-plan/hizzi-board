...
> coding-rules.md 체크리스트의 근거가 되는 상세 규칙.
...
**R1.1 — 상태 변경 코드 작성 전 반드시 `hizzi-state-flows.md`를 먼저 확인한다**
...
- 두 파일 이상 수정도 단일 블록에 묶는다. 블록 분할 금지. (프롬프트 형식 기준. 블록 내 작업 범위는 R4.11.)
...
현재 공통 컴포넌트 목록 (우선순위 순):
  ...
  🔲 ModalShell        — 모달 껍데기 (헤더/상태바/바디/푸터 M1 구조)
...
> 라우팅 트리거: CLAUDE.md [4-4] 참조

하나의 명령 블록은 동일 Phase · 동일 도메인 내로 제한한다.
그 안에서는 스텝 수 제한 없음.

**경계 판단**
- 동일 Phase — md/plan/designs/ 해당 파일의 Phase 테이블 기준
- 동일 도메인 — UI / 스키마 / 인프라 / 하네스 중 하나로 수렴
- 지원 작업(시드·스토어 확장 등)이 주 도메인에 종속되면 혼합 아님

**분할 트리거**
- Phase 경계 넘음
- 주 도메인 2개 이상 섞임 (예: UI 신규 + 스키마 변경)
- 단일 Phase 내라도 독립 검증이 필요한 작업 (예: 리팩터링 + 신규 기능)

**근거**: 세션 #35(12스텝) / #38(14스텝) 무에러의 공통분모는 스텝 수가 아니라 단일 목표 수렴.

---

### R4.12 파이프라인 적용 표기 의무

> 라우팅 트리거: 모든 명령 블록 생성 시

명령 블록 최상단 "규칙:" 줄 바로 아래에 파이프라인 적용 표기를 한 줄로 의무화한다. 표기가 비어 있거나 누락되면 오너가 "파이" 한 마디로 호출해 재검토한다.

**형식**

```
파이프라인: Step 1~N 적용 / 생략 사유: (사유)
```

**적용 값**

- **Step 1~N 적용** — 파이프라인을 탈 때. 블록 본문에 @explorer / @implementor / /codex:review / ask-claude.js 단계가 실제로 명시돼야 함
- **생략 사유: 단순 수정** — 1파일·10줄 이내 변경
- **생략 사유: MD only** — MD 파일 편집만 있고 빌드·리뷰 대상 아닐 때
- **생략 사유: 탐색/검수** — read-only 탐색·검수 블록
- **생략 사유: 기타 ({설명})** — 위에 해당 안 되는 경우 사유 명시

**금지**

- 파이프라인 표기 줄 누락
- "Step 1~N 적용" 표기 후 본문에 실제 단계 미명시
- 생략 사유 없이 빈 줄만 표기
- 세 단계(@explorer / @implementor / /codex:review) 중 일부만 쓰고 "Step 1~N 적용"으로 표기 (부분 적용 시 생략 사유: 기타로 어느 단계 생략했는지 명시)

**사례**: 2026.04.18 하네스 파이프라인 테스트 결과, 파이프라인이 관습으로만 작동하고 문서 강제력이 없어 누락 위험 확인. R4.12로 누락 가시화.
