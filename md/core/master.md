...
**거버넌스 층은 이 폴더에 없다** — 부모 폴더(`dev/`)의 저장소에 있고, 이 폴더의 MD는 전부 도메인 층이다(거버넌스 `rules.md` 문서 수정 절차 — 도메인 층). 아래 셋만 앱의 규약을 담아 자율 수정 대상이 아니다.
...
**층 `ops-ref.md` 앱별 도구 표가 이 앱의 E2E 를 "없음"으로 적는다. 그 "없음"은 부르는 줄이 없다는 뜻이지 자산이 없다는 뜻이 아니다** — 읽는 쪽이 자산으로 읽으면 이 폴더를 안 열고 지나간다(2026-08-19 에 실제로 그렇게 읽혔다).
