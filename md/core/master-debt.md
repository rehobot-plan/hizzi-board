...
>
> **그 `harness.md` 는 소거된 앱 사본이고 층 `md/core/harness.md` 가 아니다** (2026-08-19 hizzifix 에 덧붙임). **그러므로 여기 적힌 좌표를 층 좌표로 고치지 않는다** — 고치면 없어진 문서의 좌표가 살아 있는 다른 문서의 좌표로 바뀌어, 그 항목이 증언하던 사실(그때 앱 사본이 무엇을 적고 있었나)이 뒤틀린다.
>
> **가르는 값 둘** — `#9` 본문이 그 "3"을 *"git push + vercel --prod 개별 성공만 체크"* 하는 **명령 목록**으로 다루는데 층 3장은 그런 자리가 아니고, `md/archive/progress-2026-04-O.md:146` 이 *"harness.md §3 표준 배포 명령에 git push 부재"* 로 앱 사본 §3 이 그 자리였음을 따로 증언한다.
>
> **이 줄이 없어서 실제로 헛셌다** — 2026-08-18 검수가 좌표 **모양**만 보고 `#9`(145·153)를 층 장 재배치 잔존으로 세어 "잔존 5건"으로 보고했고, 그 값이 오너에게까지 올라갔다. 실제 잔존은 넷이다. **좌표는 모양이 같아도 가리키는 문서가 다를 수 있고, 그것을 가르는 것은 그 문서의 머리다.**
