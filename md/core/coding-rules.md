...
> **코드 작성 전 반드시 읽는다.**
> 상세 규칙(S1~S9): coding-rules-detail.md 참조.
...
```
상태 전환
□ state-flows.md에서 연쇄 요구사항을 확인했는가?
...
명령 블록 (MD 수정이 포함된 경우)
□ 변경 항목별 before/after 비교표가 선행 제시되어 오너 검수를 받았는가? (거버넌스 `rules.md 문서 수정 절차 — 세 층`)
□ 신규 파일 생성이면 초안 전체를 통으로 검수받았는가?
□ 부분 수정은 패치(str_replace) 방식으로 작성했는가?

명령 블록 (공통)
□ 블록 범위가 한 기능의 완결 단위인가? (CLAUDE.md [1])
□ 새 기능 / 범용 도메인 버그에 외부 라이브러리 조사를 마쳤는가? (거버넌스 하네스 1-1 탐색, R4.9)
□ Claude.ai는 파이프라인 단계·안전 규칙을 블록에 직접 나열하지 않았는가? (하네스 자동 처리 영역)
□ Claude.ai는 구현 구조·변수·함수 시그니처·API 호출 패턴을 블록에 지정하지 않았는가? (공장 1-1·1-2 영역, CLAUDE.md [1])
