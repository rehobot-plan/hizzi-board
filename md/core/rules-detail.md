...
PASS 판정은 실행 결과로만. 코드 구조 매칭·단일 케이스·구두 보고는 PASS 근거가 될 수 없다.

**4대 규칙**
1. 코드 라인 매칭 금지 — "이 줄이 있으니 OK"는 PASS 아님. 실행 경로 trace 필수
2. 시드 데이터 의무 — E2E는 전용 시드 스크립트로. 기존 데이터 의존 금지
3. Codex 실행 경로 trace — acceptance criteria마다 "어떤 입력 → 어떤 코드 경로 → 어떤 출력" 명시
4. 스크린샷 다중 케이스 — expected vs actual을 console.log로 출력, 최소 3 케이스

**우회 해법 차단**
- 명령 블록은 acceptance criteria 의무 (요구사항 = 검증 항목)
- Codex는 "요구사항 충족 여부" + "우회 vs 근본 해결" 명시적 판정
- 단일 케이스 통과 시 "미검증 경우의 수" 플래그 의무

**근거**: 세션 #7 4차 사이클. 코드 매칭 PASS + 단일 케이스 검증 + 우회 해법이 합쳐져 FAIL을 PASS로 보고.
