...
### todoRequests undefined 필드
```
증상: addDoc() called with invalid data. Unsupported field value: undefined
원인: 선택적 필드(dueDate 등)에 undefined 저장 시도

✅ 올바른 방식:
const docData: any = { ...data, status: 'pending', createdAt: serverTimestamp() };
...
```
관리자가 특정 패널에서 액션 시:
→ 해당 패널 오너 기준으로 동작해야 함
→ 관리자 이메일이 아닌 패널 오너 이메일을 fromEmail로 사용

TodoRequestBadge/Modal:
→ panelOwnerEmail props로 받아서 필터링
→ isAdmin 전체 보기 절대 금지 (전체 노출 버그 원인)
```

### 새 Firestore 컬렉션 추가 체크리스트
```
□ store 파일 생성 (initListener 포함)
□ firestore.rules 업데이트
□ firebase deploy --only firestore:rules 배포
□ undefined 필드 제거 로직 추가
□ 새 컬렉션 관련 컴포넌트에 panelOwnerEmail 같은
  컨텍스트 props 설계 (전체 노출 방지)
```
...
*업데이트: 2026.04.03 오후*
*할일 요청 기능 개발 + todoRequests undefined 버그 + 관리자 권한 설계 원칙 추가*
